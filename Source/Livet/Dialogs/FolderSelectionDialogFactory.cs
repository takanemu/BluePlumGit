		...
		public static FolderSelectionDialog CreateDialog( FolderSelectionDialogPreference preference )
		{
			switch ( preference )
			{
				...
				case FolderSelectionDialogPreference.CommonItemDialog:
				{
					if ( !CanUseCommonItemDialog )
					{
                        throw new PlatformNotSupportedException("Common Item Dialog は Windows Vista/Windows Server 2008 以降でのみサポートされています。");
					}
				...
				default:
				{
					throw new ArgumentException(
                        String.Format(CultureInfo.CurrentCulture, "サポートされない '{0}' の値 '{1}'({2},0x{2:x8}) です。", typeof(FolderSelectionDialogPreference), preference, (int)preference),
						"preference"
					);
				}
			}
